import { RouteDetail, AIRouteInsight } from '../server';

export function generateAIRouteAnalysis(routes: RouteDetail[], sourceName: string, destName: string): {
  overview: string;
  ...
  const insights: AIRouteInsight[] = routes.map(r => {
    ...
    // Advantages
    if (isFastest) advantages.push(`Fastest estimated arrival time (${r.estimatedTimeMin.toFixed(1)} min)`);
    if (isShortest) advantages.push(`Shortest physical distance (${r.distanceKm} km)`);
    if (r.traffic === 'low') advantages.push('Uncongested traffic flow with high average speed');
    if (r.safety === 'safe') advantages.push('All segments verified open and structurally sound');

    // Disadvantages
    if (!isFastest) disadvantages.push(`Takes +${(r.estimatedTimeMin - minTime).toFixed(1)} min longer than fastest alternative`);
    if (!isShortest) disadvantages.push(`Extends distance by +${(r.distanceKm - minDist).toFixed(1)} km`);
    if (r.traffic === 'heavy') disadvantages.push('Heavy congestion slowdown along major corridors');
    if (r.safety === 'dangerous') disadvantages.push('Passes near damaged infrastructure or hazard zones');
  ...
  const overview =
    `Analyzed ${routes.length} candidate emergency routes from ${sourceName} to ${destName}. ` +
