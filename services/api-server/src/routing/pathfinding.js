...
exports.ROUTE_COLORS = void 0;
exports.haversineDistanceKm = haversineDistanceKm;
exports.runAStar = runAStar;
exports.runYensKShortestPaths = runYensKShortestPaths;
exports.buildRouteDetails = buildRouteDetails;
// ─── Colors for 5 Routes ──────────────────────────────────────────────
...
// ─── A* Algorithm ──────────────────────────────────────────────────────
function runAStar(nodes, edges, sourceId, targetId, trafficProvider, excludedEdgeIds = new Set(), excludedNodeIds = new Set()) {
    ...
    while (openSet.size > 0) {
        ...
        for (const { neighborId, edge } of neighbors) {
            if (excludedNodeIds.has(neighborId))
                continue;
            const evalResult = trafficProvider.evaluateEdge(edge.id, edge.roadName, edge.distanceKm, edge.baseSpeedKmh, edge.entityId);
            if (evalResult.isBlocked)
                continue; // Cannot traverse blocked roads
...
function runYensKShortestPaths(nodes, edges, sourceId, targetId, trafficProvider, K = 5) {
    const A = [];
    const B = [];
    ...
    for (let k = 1; k < K; k++) {
        ...
        if (!prevPath || prevPath.nodeIds.length < 2)
            break;
        for (let i = 0; i < prevPath.nodeIds.length - 1; i++) {
            ...
            const spurPath = runAStar(nodes, edges, spurNode, targetId, trafficProvider, excludedEdges, excludedNodes);
            if (spurPath) {
...
function buildRouteDetails(paths, nodesMap, edgesMap, trafficProvider) {
    if (paths.length === 0)
        return [];
    ...
    return paths.map((path, idx) => {
        const routeId = idx + 1;
        const color = exports.ROUTE_COLORS[idx % exports.ROUTE_COLORS.length];
        ...
        for (let i = 0; i < path.edgeIds.length; i++) {
            ...
            const evalResult = trafficProvider.evaluateEdge(edge.id, edge.roadName, edge.distanceKm, edge.baseSpeedKmh, edge.entityId);
            if (evalResult.trafficLevel === 'heavy')
                heavyCount++;
            if (evalResult.safetyCondition === 'dangerous')
                dangerousCount++;
            segments.push({
                edgeId: edge.id,
        ...
        const traffic = heavyCount > 0 ? 'heavy' : segments.some(s => s.penaltyMultiplier > 1.2) ? 'moderate' : 'low';
        // Determine overall Safety Condition
        const safety = dangerousCount > 0 ? 'dangerous' : segments.some(s => s.penaltyMultiplier > 1.2) ? 'cautious' : 'safe';
